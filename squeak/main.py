...
def remap_right(array):
    ...
    return array / np.sign(array[-1])


def uniform_time(x, t, interval=20, max_duration=5000):
    ...
    max_duration : int, optional
        Length to extend to.
        Note: Currently crashes if max(t) > max_duration
        Default 5000
        
    Returns
    ---------
    uniform_time_coordinates : coordinates extended up to max_duration"""
    timepoints = np.arange(0, max_duration+.01, interval)
    coordinates = np.interp(timepoints, t, x)
    return pd.Series(coordinates, timepoints)
